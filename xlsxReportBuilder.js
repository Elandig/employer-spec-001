...
function replaceLegacyStyles(style) {
  for (const key in style) {
    ...
    if (typeof style[key] === "string" && key.toLowerCase().includes("color")) {
      style[key] = { rgb: style[key].startsWith('#') ? style[key].slice(1): style[key] };
    }
  ...
  sheets.forEach(sheet => {
    ...
    sheet.data.forEach((row, rowno) => {
      Object.keys(sheet.specification).forEach((colname, colno) => {
        ...
        if (res.merges) {
          let index = "R" + (headrow + rowno - res.merges.up) + "C" + (colno - res.merges.left);
          if (merges[index] && (merges[index].e.r < headrow + rowno || merges[index].e.c < colno) ) {
              merges[index].e = { r: headrow + rowno, c: colno };
          } else {
            merges[index] = {
                s: { r: headrow + rowno - res.merges.up, c: colno - res.merges.left },
    ...
    colwidths.forEach((width, key) => ws["!cols"][key] = { wch: width + 0.2 })
